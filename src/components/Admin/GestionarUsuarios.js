...
const editarPaciente = () => {
  clienteAxios.put('http://25.0.53.159:3000/api/users/updateInfo/' + correoU, paciente)
    .then(({ data }) => {
...
const editarMedico = () => {
  clienteAxios.put('http://25.0.53.159:3000/api/users/updateInfoMedico/' + correoU, medico)
    .then(({ data }) => {
      console.log(data.msg);
      Swal.fire({
        icon: 'success',
  ...
  const validateForm = (busqueda) =>{
   ...
   const buscarPaciente = ()=>{
    ...
    if(initialForm.rol == 'P') {
      clienteAxios.get('http://25.0.53.159:3000/api/users/userInfo/'+busqueda)
      .then(({data})=>{
      ...
      clienteAxios.get('http://25.0.53.159:3000/api/users/userMedicoInfo/'+busqueda)
      .then(({data})=>{
        console.log(data)
        setMedico((prevState) => ({
