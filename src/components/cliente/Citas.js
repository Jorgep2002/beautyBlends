...
function Citas (props){
   ...
   const getCitas = ()=>{
    ...
    clienteAxios.get('http://25.0.53.159:3000/api/citas/'+initialForm.area)
    .then(({data})=>{
...
return (
  <span>
    ...
    const agregarCita = (id_cita) =>{
        console.log(id_cita)
        clienteAxios.defaults.headers.common['Authorization'] = `Bearer ${props.token}`;
    clienteAxios.put('http://25.0.53.159:3000/api/citas/asignarCita/'+id_cita,parametroUser)
    .then(({data})=>{
      console.log(data)
                 Swal.fire({
           icon: 'success',
    ...
    const renderData = (data) =>{
      return(
        data.length === 0 ? (
          <p style={{textAlign:"center"}}>No hay citas para el área seleccionada</p>
        ) : (
              ...
              return (
                <div class="cajas_citas" key={cita.cita_id}>
                  <div class="flex_descripcion">
                    ...
                    <button class="agregar-cita-button" onClick={()=> agregarCita(cita.id_cita)}>Asignar</button>
                  </div>
                </div>
              );
            })}
    ...
    return(
        ...
        <div class="campo-registro">
            <select required  onChange={handleChange} name="area" className="Especialidad select-custom"  id="" value={initialForm.area} >

            <option  selected disabled  >Area de tratamiento</option>
